...
// --- Config ---
const API_KEY = import.meta.env.VITE_TMDB_API_KEY; // set in .env / deployment settings
...
function Card({ image, title, subtitle, onClick, footer, badge }) {
  return (
    <button
      onClick={onClick}
...
function Grid({ children }) {
  return (
    <div className="grid gap-4 grid-cols-[repeat(auto-fill,minmax(160px,1fr))]">{children}</div>
  );
}
...
export default function App() {
  ...
  const [seedMovie, setSeedMovie] = useState(null); // movie used to compute recommendations
  ...
  // Recommendations update when seed movie changes
  useEffect(() => {
    if (!seedMovie) return;
  ...
  // Recommended TV update when selectedTV changes
  useEffect(() => {
  ...
  // Load TV details + default season episodes
  useEffect(() => {
    if (!selectedTV) return;
    (async () => {
      const tv = await j(`${TMDB}/tv/${selectedTV.id}?api_key=${API_KEY}`);
      // default to S1 (or first season that has episodes)
      const defaultSeason = (tv.seasons?.find((s) => s.season_number === 1) || tv.seasons?.[0])?.season_number || 1;
      setSelectedTV(tv);
  ...
  useEffect(() => {
    ...
    j(`${TMDB}/tv/${selectedTV.id}/season/${selectedSeason}?api_key=${API_KEY}`)
  ...
  const chooseTV = (tv) => {
    setSelectedTV(tv);
    // smooth-scroll to the series explorer
    requestAnimationFrame(() => {
      seriesExplorerRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
    });
  };
  ...
  const seasons = useMemo(() => selectedTV?.seasons?.filter((s) => s.season_number !== 0) || [], [selectedTV]);
  ...
  return (
    <div className="min-h-screen text-white bg-gradient-to-b from-neutral-950 via-neutral-900 to-neutral-950">
      <div className="max-w-7xl mx-auto px-4 pt-6 pb-24">
          ...
          <div className="max-w-7xl mx-auto">
            <div className="relative">
              ...
              {suggestions.length > 0 && (
                <div
                  ref={sugBoxRef}
                  className="absolute mt-2 w-full rounded-xl overflow-hidden border border-white/10 bg-neutral-900 shadow-2xl"
                >
                  {suggestions.map((s, i) => (
                    <button
        ...
        {/* Series Explorer (selection of season + episodes) */}
            ...
            {selectedTV ? (
              <div className="space-y-4">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <Badge>
                    {selectedTV.number_of_seasons} season{selectedTV.number_of_seasons === 1 ? "" : "s"}
                  </Badge>
                ...
                <Grid>
                  {episodes.map((ep) => (
                    <div key={ep.id} className="rounded-xl overflow-hidden bg-white/5 border border-white/10">
                      ...
                      <div className="p-3">
                        ...
                        {ep.overview && (
                          <p className="text-xs text-white/70 line-clamp-2 mt-1">{ep.overview}</p>
                        )}
                      </div>
                    </div>
                  ))}
              ...
              <div className="text-white/70 text-sm">Select a series from the sections below to explore seasons and episodes.</div>
            )}
          </Section>
        </div>
        ...
        {/* Recommended Movies (based on last clicked) */}
        <Section
          title="Recommended Movies"
          right={seedMovie && (
            <div className="text-sm text-white/70">Based on <span className="font-medium">{seedMovie.title}</span></div>
          )}
        >
          <Grid>
        ...
        {/* Recommended Series (based on chosen) */}
        <Section
          title="Recommended Series"
          right={selectedTV && (
            <div className="text-sm text-white/70">Because you chose <span className="font-medium">{selectedTV.name}</span></div>
          )}
        >
          <Grid>
      ...
      <Modal open={playerOpen} onClose={() => setPlayerOpen(false)} title={playerTitle}>
        {currentTMDBId ? (
          ...
          <div className="w-full h-full grid place-items-center">Missing TMDB id</div>
        )}
      </Modal>
